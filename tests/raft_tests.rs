...
fn disk_id(node: usize) -> ActorId { ActorId((node * 10 + 1) as u64) }
fn wal_id(node: usize)  -> ActorId { ActorId((node * 10 + 2) as u64) }
fn txn_id(node: usize)  -> ActorId { ActorId((node * 10 + 3) as u64) }
fn raft_id(node: usize) -> ActorId { ActorId((node * 10 + 4) as u64) }
...
fn make_cluster(n: usize, seed: u64) -> (MessageBus, DatabaseConfig) {
    ...
    for i in 0..n {
        ...
        bus.register(Box::new(WalWriter::new(wal_id(i), disk_id(i), txn_id(i), &cfg)));
        bus.register(Box::new(TransactionManager::new(txn_id(i), wal_id(i))));

        let raft_seed = seed.wrapping_add(i as u64 * 1000);
        let mut server = RaftServer::new(raft_id(i), format!("node{}", i), txn_id(i), &cfg, raft_seed);
        for j in 0..n {
            if i != j { server.add_peer(format!("node{}", j), raft_id(j)); }
        }
        raft_servers.push(server);
    }
    for server in raft_servers { bus.register(Box::new(server)); }
...
fn raft_single_node_becomes_leader() {
    ...
    assert_eq!(*raft.role(), ServerRole::Leader, "Single node must become leader");
}
...
fn raft_leader_sends_heartbeats() {
    ...
    let trace = bus.trace();
    let heartbeats = trace[before..].iter().filter(|(_, _, _, msg)| {
        msg.contains("RaftAppendEntries")
    }).count();
    assert!(heartbeats > 0, "Leader should send heartbeats, found {}", heartbeats);
}
...
fn raft_log_replicated_to_followers() {
    ...
    let leader_log = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().log_last_index();
    assert!(leader_log >= 1, "Leader should have >= 1 log entry, got {}", leader_log);

    for i in 0..3 {
        if i == leader_idx { continue; }
        let f_log = bus.actor::<RaftServer>(raft_id(i)).unwrap().log_last_index();
        assert_eq!(f_log, leader_log, "Follower {} log {} should match leader {}", i, f_log, leader_log);
    }
}
...
fn raft_commit_requires_majority() {
    ...
    let commit = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;
    assert!(commit >= 1, "Entry should be committed after majority ack (commit={})", commit);
}
...
fn raft_client_redirect_on_follower() {
    ...
    let redirect = client.received.iter().find(|m| matches!(m, Message::RaftRedirect { .. }));
    assert!(redirect.is_some(), "Follower should RaftRedirect");

    if let Some(Message::RaftRedirect { leader_id }) = redirect {
        assert_eq!(leader_id.as_deref(), Some(format!("node{}", leader_idx).as_str()));
    }
}
...
fn raft_partition_prevents_commit() {
    ...
    let leader_idx = find_leader(&bus, 3).expect("No leader");
    let commit_before = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;
    ...
    for i in 0..3 {
        if i != leader_idx {
            ...
            let b = format!("node{}", i);
            bus.send(CLIENT_ID, raft_id(leader_idx), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
            bus.send(CLIENT_ID, raft_id(i), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
        }
    }
    ticks(&mut bus, 50);
    ...
    let commit_after = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;
...
fn raft_new_leader_after_partition() {
    ...
    for i in 0..3 {
        if i != old_leader {
            ...
            let b = format!("node{}", i);
            bus.send(CLIENT_ID, raft_id(old_leader), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
            bus.send(CLIENT_ID, raft_id(i), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
        }
    }
...
fn raft_stale_leader_rejected() {
    ...
    let old_term = bus.actor::<RaftServer>(raft_id(old_leader)).unwrap().current_term;
    ...
    for i in 0..3 {
        if i != old_leader {
            let a = format!("node{}", old_leader);
            let b = format!("node{}", i);
            bus.send(CLIENT_ID, raft_id(old_leader), Message::NetHeal { node_a: a.clone(), node_b: b.clone() }, 0);
            bus.send(CLIENT_ID, raft_id(i), Message::NetHeal { node_a: a.clone(), node_b: b.clone() }, 0);
        }
    }
    ticks(&mut bus, 200); // Let old leader receive higher-term messages.

    let new_role = bus.actor::<RaftServer>(raft_id(old_leader)).unwrap().role().clone();
    let new_term = bus.actor::<RaftServer>(raft_id(old_leader)).unwrap().current_term;
    assert_ne!(new_role, ServerRole::Leader, "Old leader should step down");
    assert!(new_term > old_term, "Old leader term should have advanced ({} → {})", old_term, new_term);
}
...
fn raft_log_catch_up_after_rejoin() {
    ...
    bus.send(CLIENT_ID, raft_id(leader_idx), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
    bus.send(CLIENT_ID, raft_id(follower_idx), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
    ticks(&mut bus, 20);
    ...
    let leader_commit = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;

    // Heal.
    bus.send(CLIENT_ID, raft_id(leader_idx), Message::NetHeal { node_a: a.clone(), node_b: b.clone() }, 0);
    bus.send(CLIENT_ID, raft_id(follower_idx), Message::NetHeal { node_a: a.clone(), node_b: b.clone() }, 0);
    ticks(&mut bus, 200);

    let follower_log = bus.actor::<RaftServer>(raft_id(follower_idx)).unwrap().log_last_index();
    assert!(follower_log >= leader_commit,
        "Follower should catch up (follower_log={}, leader_commit={})", follower_log, leader_commit);
}
...
fn raft_snapshot_install_for_lagged_follower() {
    ...
    let leader_commit = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;
    let leader_term  = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().current_term;
    ...
    let follower_commit = bus.actor::<RaftServer>(raft_id(follower_idx)).unwrap().commit_index;
    assert!(follower_commit >= leader_commit,
        "Follower should have applied snapshot (follower={}, leader={})", follower_commit, leader_commit);
}
...
fn raft_transaction_survives_leader_change() {
    ...
    let commit_before = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;
    assert!(commit_before >= 1, "Txn should be committed (commit={})", commit_before);
    ...
    let new_leader = (0..3).find(|&i| {
        i != leader_idx && bus.actor::<RaftServer>(raft_id(i))
            .map(|r| *r.role() == ServerRole::Leader)
            .unwrap_or(false)
    });
    assert!(new_leader.is_some(), "New leader should be elected");

    let new_commit = bus.actor::<RaftServer>(raft_id(new_leader.unwrap())).unwrap().commit_index;
    assert!(new_commit >= 1,
        "New leader should have the committed entry (new_commit={})", new_commit);
}
...
fn raft_five_node_tolerates_two_failures() {
    ...
    for &f in &failed {
        for i in 0..5 {
            if i != f {
                let a = format!("node{}", f);
                let b = format!("node{}", i);
                bus.send(CLIENT_ID, raft_id(f), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
                bus.send(CLIENT_ID, raft_id(i), Message::NetPartition { node_a: a.clone(), node_b: b.clone() }, 0);
            }
        }
    }
    ...
    let commit = bus.actor::<RaftServer>(raft_id(leader_idx)).unwrap().commit_index;
    assert!(commit >= 1, "5-node cluster should commit with 2 failures (commit={})", commit);
}
...
fn raft_chaos_fuzz() {
    for seed in 0u64..10 {
        let run_trace = |s: u64| -> Vec<String> {
            ...
            let fault = FaultConfig { drop_probability: 0.2, ..FaultConfig::none() };
        ...
        assert_eq!(trace_a, trace_b,
            "Chaos fuzz seed={}: determinism regression", seed);
    }
}
