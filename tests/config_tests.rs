...
use rust_dst_db::config::{
    BTREE_DATA_FILE_ID, BTREE_MAX_INTERNAL_KEYS, BTREE_MAX_LEAF_ENTRIES, DEFAULT_BUFFER_POOL_PAGES,
    DEFAULT_FAULT_PROBABILITY, DEFAULT_QUEUE_DEPTH, DatabaseConfig, NET_DEFAULT_PORT, PAGE_SIZE,
    RAFT_ELECTION_TIMEOUT_MAX_TICKS, RAFT_ELECTION_TIMEOUT_MIN_TICKS,
    RAFT_HEARTBEAT_INTERVAL_TICKS, RAFT_MAX_ENTRIES_PER_APPEND, SIM_DISK_READ_LATENCY_TICKS,
    SIM_DISK_WRITE_LATENCY_TICKS, SIM_FSYNC_LATENCY_TICKS, SIM_NETWORK_LATENCY_TICKS,
    SIM_NETWORK_LATENCY_VARIANCE_TICKS, WAL_HEADER_SIZE, WAL_MAX_RECORD_SIZE,
};
...
fn default_config_matches_constants() {
    ...
    assert_eq!(
        cfg.sim_disk_write_latency_ticks,
        SIM_DISK_WRITE_LATENCY_TICKS
    );
    assert_eq!(cfg.sim_network_latency_ticks, SIM_NETWORK_LATENCY_TICKS);
    assert_eq!(
        cfg.sim_network_latency_variance_ticks,
        SIM_NETWORK_LATENCY_VARIANCE_TICKS
    );
    ...
    assert_eq!(
        cfg.raft_election_timeout_min,
        RAFT_ELECTION_TIMEOUT_MIN_TICKS
    );
    assert_eq!(
        cfg.raft_election_timeout_max,
        RAFT_ELECTION_TIMEOUT_MAX_TICKS
    );
...
fn custom_btree_limits_respected() {
    ...
    let puts_ok = client
        .received
        .iter()
        .filter(|m| matches!(m, Message::BTreePutOk))
        .count();
    assert_eq!(
        puts_ok, 10,
        "All 10 puts should succeed with custom split limit"
    );
}
...
fn custom_disk_latency_affects_timing() {
    ...
    let client = bus.actor::<Collector>(CLIENT_ID).unwrap();
    let reads_before = client
        .received
        .iter()
        .filter(|m| matches!(m, Message::DiskReadOk { .. }))
        .count();
    ...
    assert_eq!(
        ...
        "DiskReadOk should not arrive before latency ticks elapsed"
    );
    ...
    assert_eq!(
        reads_after,
        reads_before + 1,
        "DiskReadOk should arrive after latency ticks"
    );
}
...
fn custom_network_latency_affects_timing() {
    ...
    assert!(
        ...
        "NetRecv should not arrive before latency={} ticks",
        latency
    );
    ...
    assert_eq!(
        peer.received.len(),
        1,
        "NetRecv should arrive after latency={} ticks",
        latency
    );
}
...
fn zero_latency_config_delivers_immediately() {
    ...
    let client = bus.actor::<Collector>(CLIENT_ID).unwrap();
    let writes_ok = client
        .received
        .iter()
        .filter(|m| matches!(m, Message::DiskWriteOk { .. }))
        .count();
    assert_eq!(
        writes_ok, 1,
        "DiskWriteOk should arrive in same tick wave with zero latency"
    );
}
...
fn two_buses_different_configs() {
    let cfg_small = DatabaseConfig {
        buffer_pool_pages: 4,
        ..DatabaseConfig::default()
    };
    let cfg_large = DatabaseConfig {
    ...
    assert!(
        pool1.cached_count() <= 4,
        "Small bus pool should have evicted pages"
    );
    assert_eq!(
        pool2.cached_count(),
        8,
        "Large bus pool should hold all 8 pages"
    );
}
...
fn wal_file_id_from_config() {
    ...
    let ok = client
        .received
        .iter()
        .any(|m| matches!(m, Message::WalAppendOk { .. }));
    assert!(ok, "WalAppendOk expected");

    // Data should be on file_id=5 in the disk.
    let disk = bus.actor::<SimDisk>(DISK_ID).unwrap();
    assert!(
        disk.file_contents(5).is_some(),
        "WAL data should be on file_id=5"
    );
    assert!(
        disk.file_contents(0).is_none(),
        "Nothing should be on file_id=0"
    );
}
