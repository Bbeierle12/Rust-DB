...
fn create_index_basic() {
    ...
    db.execute_sql("CREATE TABLE users (id BIGINT NOT NULL, name TEXT, age BIGINT)")
        .unwrap();
...
fn create_unique_index() {
    ...
    let db = Database::open(&dir).unwrap();
    db.execute_sql("CREATE TABLE users (id BIGINT NOT NULL, email TEXT)")
        .unwrap();
...
fn insert_maintains_index() {
    ...
    let idx = db.get_index("idx_users_name").unwrap().unwrap();
    let rows = db
        .index_lookup(&schema, &idx, &[Value::Text("Alice".to_string())])
        .unwrap();
...
fn update_maintains_index() {
    ...
    db.execute_sql("UPDATE users SET name = 'Alicia' WHERE id = 1")
        .unwrap();
    ...
    let new_rows = db
        .index_lookup(&schema, &idx, &[Value::Text("Alicia".to_string())])
        .unwrap();
...
fn delete_maintains_index() {
    ...
    let rows = db
        .index_lookup(&schema, &idx, &[Value::Text("Bob".to_string())])
        .unwrap();
    assert_eq!(rows.len(), 1);
}
...
fn unique_index_rejects_duplicate() {
    ...
    db.execute_sql("INSERT INTO users (id, email) VALUES (1, 'alice@test.com')")
        .unwrap();
    ...
    match err {
        DbError::Constraint(msg) => {
            assert!(
                ...
                "expected unique index error, got: {}",
                msg
            );
        }
...
fn index_lookup_returns_correct_rows() {
    ...
    let db = Database::open(&dir).unwrap();
    db.execute_sql("CREATE TABLE products (id BIGINT NOT NULL, category TEXT, name TEXT)")
        .unwrap();
    db.execute_sql("CREATE INDEX idx_products_category ON products (category)")
        .unwrap();

    db.execute_sql("INSERT INTO products (id, category, name) VALUES (1, 'electronics', 'laptop')")
        .unwrap();
    db.execute_sql("INSERT INTO products (id, category, name) VALUES (2, 'electronics', 'phone')")
        .unwrap();
    db.execute_sql("INSERT INTO products (id, category, name) VALUES (3, 'books', 'novel')")
        .unwrap();
    ...
    let electronics = db
        .index_lookup(&schema, &idx, &[Value::Text("electronics".to_string())])
        .unwrap();
    assert_eq!(electronics.len(), 2);

    let books = db
        .index_lookup(&schema, &idx, &[Value::Text("books".to_string())])
        .unwrap();
    assert_eq!(books.len(), 1);
    assert_eq!(
        books[0].get("name"),
        Some(&Value::Text("novel".to_string()))
    );
}
...
fn multi_column_index() {
    ...
    let db = Database::open(&dir).unwrap();
    db.execute_sql("CREATE TABLE orders (id BIGINT NOT NULL, customer TEXT, status TEXT)")
        .unwrap();
    db.execute_sql("CREATE INDEX idx_orders_cust_status ON orders (customer, status)")
        .unwrap();

    db.execute_sql("INSERT INTO orders (id, customer, status) VALUES (1, 'Alice', 'shipped')")
        .unwrap();
    db.execute_sql("INSERT INTO orders (id, customer, status) VALUES (2, 'Alice', 'pending')")
        .unwrap();
    db.execute_sql("INSERT INTO orders (id, customer, status) VALUES (3, 'Bob', 'shipped')")
        .unwrap();
...
fn index_survives_wal_recovery() {
...

    {
        ...
        assert!(
            idx.is_some(),
            "index definition should survive WAL recovery"
        );
        let idx = idx.unwrap();
        assert_eq!(idx.table, "users");
        assert_eq!(idx.columns, vec!["name".to_string()]);
...
fn list_indexes_for_table() {
    ...
    let db = Database::open(&dir).unwrap();
    db.execute_sql("CREATE TABLE users (id BIGINT, name TEXT, email TEXT)")
        .unwrap();
    db.execute_sql("CREATE TABLE orders (id BIGINT, total BIGINT)")
        .unwrap();
    ...
    db.execute_sql("CREATE UNIQUE INDEX idx_users_email ON users (email)")
        .unwrap();
    db.execute_sql("CREATE INDEX idx_orders_total ON orders (total)")
        .unwrap();
